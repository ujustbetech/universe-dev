...
const FacilitatorSection = (props) => {
  const [sections, setSections] = useState(props?.data?.facilitatorSections || []);
...
useEffect(() => {
  ...
  const handleAddSection = () => {
    setSections([...sections, { id: Date.now(), facilitator: '', facilitatorDesc: '' }]);
    ...
    if (sectionToRemove.facilitator) {
      try {
        ...
        if (docSnap.exists()) {
          ...
          const updatedFacilitators = data.facilitatorSections.filter((_, i) => i !== index);

          await updateDoc(eventRef, { facilitatorSections: updatedFacilitators });
            ...
            <div key={section.id} className='formBoxCon'>
              ...
              <div className='autosuggest'>
                <input
                  ...
                  value={facilitatorSearch[index] || ''}
                  onChange={(e) => handleSearchFacilitator(index, e.target.value)}
                />
  ...
  <ul className="dropdown">
    {filteredUsers[index].map(user => (
      <li
        key={user.id}
        onClick={() => handleSelectFacilitator(index, user.name)}
      >
        {user.name}
      </li>
    ))}
  ...
  <span class="tooltiptext">Remove</span>
      ...
      <ul>
        ...
        <button className="submitbtn" onClick={updateFacilitatorSections}>Save</button>
        </div>
       </li>
       </ul>
      </div>
